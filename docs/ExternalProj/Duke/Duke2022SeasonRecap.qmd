...
## Attack Penalty Corners

```{r}
penalty_corners <- read_csv("DukeData/PenaltyCornerData/APC CSV.csv")

penalty_corners <-  penalty_corners |>  # get rid of columns with only null values
  select(where(~ sum(is.na(.x)) < nrow(penalty_corners)))

```

Duke had `r nrow(penalty_corners)` penalty corners throughout the year.

## How do We Score Goals?

```{r}
num_instances <- penalty_corners |> 
  drop_na(`1st Phase`) |> 
  nrow()

goal_first <- penalty_corners |> 
  select(`1st Phase`) |> 
  filter(grepl("Goal", `1st Phase`))
```

Out of the `r nrow(penalty_corners)` penalty corners recorded, there is recorded data for the `first phase` for only `r num_instances` penalty corners. Counts of goals and other metrics may be below the actual number.

For instance, there were only `r nrow(goal_first)` recorded goals, yet we know that Duke scored more penalty corner goals than this number.
...
```{python}
#| label: unique-values
penalty_corners = r.penalty_corners
routine = penalty_corners["Routine"]
traps = penalty_corners["Traps"]


  
set_routine = setIze(routine)
set_traps = setIze(traps)

routine_formula = form(set_routine)
traps_formula = form(set_traps)
traps_formula
```

Can we use Routine and Traps to see how Duke scores?
...
penalty_corners <- penalty_corners |> 
  mutate(Goal = if_else(grepl("Goal", `1st Phase`), "Yes", "No"))

penalty_corners <- penalty_corners |> 
  mutate(Left_Option = if_else(grepl("Left Option", Routine), 1, 0),
         L1_Tip = if_else(grepl("L1 Tip", Routine), 1, 0),
         vs_3_Def = if_else(grepl("vs 3 Def", Routine), 1, 0),
         Run = if_else(grepl("Run", Routine), 1, 0),
         Early_Break = if_else(grepl("Early Break", Routine), 1, 0),
         Right_Option = if_else(grepl("L1 Tip", Routine), 1, 0),
         Flick = if_else(grepl("Flick", Routine), 1, 0),
         Broken_Play = if_else(grepl("Broken Play", Routine), 1, 0),
         Shot = if_else(grepl("Shot", Routine), 1, 0),
         R1_Tip = if_else(grepl("R1 Tip", Routine), 1, 0),
         Giver_Tip = if_else(grepl("Giver Tip", Routine), 1, 0),
         Back_to_Giver = if_else(grepl("Back to Giver", Routine), 1, 0),
         Single = if_else(grepl("Single", Routine), 1, 0),
         Trap_2 = if_else(grepl("Trap 2", Routine), 1, 0),
         Short_Push = if_else(grepl("Short Push", Routine), 1, 0),
         Moving = if_else(grepl("Moving", Routine), 1, 0),
         Trap_1 = if_else(grepl("Trap 1", Routine), 1, 0))


routine_formula <- formula(Goal ~ py$routine_formula[[1]])
traps_formula <- formula(Goal ~ py$traps_formula)

tree1 <- rpart(Goal ~ Left_Option + L1_Tip + vs_3_Def + Run +Early_Break + Right_Option + Flick + Broken_Play + Shot + R1_Tip + Giver_Tip + Back_to_Giver, data = penalty_corners)

tree2 <- rpart(Goal ~ Single + Trap_2 + Short_Push + Moving + Trap_1, data = penalty_corners)
```
